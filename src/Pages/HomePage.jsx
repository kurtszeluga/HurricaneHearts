      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-3">
        ...
        <div className="flex flex-wrap items-stretch gap-2">
          <button
            ...
            className={
              activeEvent
                ? "bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-xl font-semibold text-xs sm:text-sm leading-tight"
                : "bg-gray-200 text-gray-400 px-3 py-2 rounded-xl font-semibold text-xs sm:text-sm leading-tight cursor-not-allowed"
            }
          >
            Request Assistance
          </button>

          <button onClick={() => onGoToRequests({ type: "status", value: "Open" })} className="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            Open: {openRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "status", value: "Assigned" })} className="bg-yellow-50 hover:bg-yellow-100 text-yellow-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            Assigned: {assignedRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "special", value: "Partially Claimed" })} className="bg-orange-50 hover:bg-orange-100 text-orange-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            Partial: {partiallyClaimedRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "status", value: "Completed" })} className="bg-green-50 hover:bg-green-100 text-green-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            Completed: {completedRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "status", value: "Cancelled" })} className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            Cancelled: {cancelledRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "mine", value: "My Requests" })} className="bg-purple-50 hover:bg-purple-100 text-purple-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
            My Requests: {myRequests.length}
          </button>

          <button onClick={() => onGoToRequests({ type: "mine", value: "My Claims" })} className="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-3 py-2 rounded-xl text-xs sm:text-sm font-semibold">
